...
public class StrUtils {
  ...
  //子字符串modelStr在字符串str中第count次出现时的下标
     ...
     */
  	...
  	/**
     *	根据字符串表达式计算结果，返回boolean结果
     * @param express
     * @return
     * @throws ScriptException
     */
    public static boolean eval(String express, String type) throws ScriptException {
    	Object result;
    	if("date".equals(type)) {
    		SimpleDateFormat HHmm = new SimpleDateFormat("HH:mm");
    		String date = HHmm.format(new Date(System.currentTimeMillis()));
    		express = express.replace("date", "'"+date+"'");
    	}
		result = engine.eval(express);
		if(null != result) {
			boolean match = (boolean) result; 
			return match;
		}
		return false;
    }
    ...
    public static void main(String[] args) throws ScriptException{
        ...
        boolean aa = eval("date < '12:00'", "date");
